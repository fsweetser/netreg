...
# 
# $Id: RandomLB.pm,v 1.3 2008/03/27 19:42:40 vitroth Exp $
#
# $Log: RandomLB.pm,v $
# Revision 1.3  2008/03/27 19:42:40  vitroth
# Merging changes from duke merge branch to head, with some minor type corrections
# and some minor feature additions (quick jump links on list pages, and better
# handling of partial range allocations in the subnet map)
#
# Revision 1.2.22.1  2007/10/11 20:59:45  vitroth
...
#
#
# Revision 1.2  2002/03/31 04:13:51  kevinm
# * Couple fixes, now it even works. :)
#
# Revision 1.1  2002/03/31 03:37:53  kevinm
# * Implementation of the reference implementation collector
#
#
#
...
sub _calc_UI {
  my ($self) = @_;
  foreach my $M (keys %{$self->{machines}}) {
    $self->{machines}->{$M}->{UsabilityIndex} = int(rand(100));
  }
}
...
DNS::RandomLB - Reference implementation of collector agent for C<DNS::LBPool>

=head1 SYNOPSIS

  use DNS::LBPool;
  my $pool = new DNS::LBPool;
  $pool->AddPool('Random1', 'RandomLB');
...
Creates a new C<DNS::RandomLB>. Should be instantiated via LBPool only. $Name specifies
