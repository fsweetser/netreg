...
package CMU::WebInt::S_P_status::Blade::B3500_48;
use strict;
use warnings;
...
sub init {
  ...
  foreach $i (1 .. $self->{'vars'}{'port_cnt'} ) {
    ...
    $self->{'ports'}[$i]->init( 'parent' => $self, 'port_num' => ($i + 1), 'label' => $i, 'dev_conf' => $self->{'dev_conf'}{'children'}[$i]);
  }
