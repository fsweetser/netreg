...
package CMU::WebInt::S_P_status::Panel::Mixed;
use strict;
use warnings;
...
sub init {
  ...
  $self->{'vars'}{'IBM'} = CMU::WebInt::S_P_status::Panel::IBM->new();
  $self->{'vars'}{'Cat5'} = CMU::WebInt::S_P_status::Panel::Cat5->new();
  $self->{'vars'}{'IBM'}->init(%params);
  $self->{'vars'}{'Cat5'}->init(%params);
...
#  $self->{'parent'} = "My Parent";
#  warn __FILE__ . ":" . __LINE__ . ": I am " . Data::Dumper->Dump([$self],[qw(self)]) . "\n";
#  $self->{'parent'} = $parent;



  $val .= $self->{'vars'}{'IBM'}->display(%params) unless ($self->{'vars'}{'IBM'}->isEmpty());
  $val .= $self->{'vars'}{'Cat5'}->display(%params) if ($self->{'vars'}{'IBM'}->isEmpty() || !$self->{'vars'}{'Cat5'}->isEmpty()) ;
...
sub SNMP_init {
  ...
  $self->{'vars'}{'IBM'}->SNMP_init(%params);
  $self->{'vars'}{'Cat5'}->SNMP_init(%params);
}
...
sub netreg_init {
  ...
  $self->{'vars'}{'IBM'}->netreg_init(%params);
  $self->{'vars'}{'Cat5'}->netreg_init(%params);
}
...
sub isEmpty {
  ...
  if ($self->{'vars'}{'IBM'}->isEmpty() && $self->{'vars'}{'Cat5'}->isEmpty()) {
    return(1);
  } else {
    return(undef);
  }
