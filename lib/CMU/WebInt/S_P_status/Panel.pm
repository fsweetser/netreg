...
package CMU::WebInt::S_P_status::Panel;
use strict;
use warnings;
...
use CMU::WebInt::S_P_status::vars qw( $panel );

# require superclass


# Use subclasses
use CMU::WebInt::S_P_status::Panel::IBM;
use CMU::WebInt::S_P_status::Panel::Cat5;
use CMU::WebInt::S_P_status::Panel::Cat5_48;
use CMU::WebInt::S_P_status::Panel::Mixed;
...
sub dev_from_port {
  ...
  my (%params) = @_;
  my ($port) = $params{'port'};
  my ($data, $dapos)  = $self->panel_data();
  my (@hits, $name);

#FIXME - To do multiple panels, this will have to use bldg, closet, rack and panel as well as port.  
  return (undef) if (length($port) ne 2);
  @hits = grep {$_->[$dapos->{'cable.label_from'}] =~ /$self->{'vars'}{'panel_num'}-$port$/} @$data;

  if ((scalar @hits) != 1) {
# FIXME - here is where would ask our parent for help.
    warn __FILE__ . ":" . __LINE__ . ": Non unique outlet sought\n";
    warn __FILE__ . ":" . __LINE__ . ": " . scalar (@hits) . " Rows found\n" . Data::Dumper->Dump([\@hits],[qw(hits)]) . "\n"; ;
    return (undef) 
  }
  if (! defined $hits[0][$dapos->{'outlet.device'}]) {
    return (undef);
  }
  
  $name = $self->s_p_name( 'vpid' => $hits[0][$dapos->{'outlet.device'}]);
  return($name);
...
#FIXME - To do multiple panels, this will have to use bldg, closet, rack and panel as well as port.  
  ...
  if ((scalar @hits) != 1) {
    warn __FILE__ . ":" . __LINE__ . ": Non unique outlet sought\n";
#FIXME - Here we ask our parent...
    return (undef) 
  }
  if (! defined $hits[0][$dapos->{'outlet.port'}]) {
    warn __FILE__ . ":" . __LINE__ . ": No outlet.device defined\n";
    return (undef);
  }
  return($hits[0][$dapos->{'outlet.port'}]);
