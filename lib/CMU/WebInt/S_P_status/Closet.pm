...
package CMU::WebInt::S_P_status::Closet;
use strict;
use warnings;
...
#  $val .= Data::Dumper->Dump([$self],[qw(self)]) . "\n"; 
  ...
  if ($self->{'vars'}{'rack_count'} > 1) {
    $head = "<table border=\"6\"><tr><th>Closet $self->{'vars'}{'closet_num'}</th></tr><td>\n";
    $tail = "</td></tr></table>\n";
  }
  $val .= $head;
  foreach $i (1 .. $self->{'vars'}{'rack_count'}) {
    $val .= $self->{'racks'}[$i]->display(%params);
  }
  $val .= $tail;
  return($val);
}
...
#  warn __FILE__ . ":" . __LINE__ . ": Closet::init called by " . join(" <:> ", caller(0)) . " with " . join ("\n", map { Data::Dumper->Dump([$params{$_}],["params{$_}"]) if ($_ ne 'parent') } (keys %params)) . "\n";

  $self->{'parent'} = $params{'parent'};

  if (($params{'rack'} =~ /^[0-9A-Za-z]$/)) {
    $self->{'vars'}{'rack_count'} = 1;
  } else {
    warn __FILE__ . ":" . __LINE__ . ": Multiple racks not supported\n";
    return
  }
  $self->{closet_num} = $params{'clos'};
  
  foreach ( 1 .. 1 ) {
    $self->{'racks'}[$_] = CMU::WebInt::S_P_status::Rack->new();
    $self->{'racks'}[$_]->init(
			       rack => $params{'rack'},
			       pane => $params{'pane'},
			       parent => $self
...
# FIXME This is where multiple rack support needs to be added
  }
  return($panel_type);
}
