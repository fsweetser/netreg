...
package CMU::WebInt::S_P_status::Device::D6509;
use strict;
use warnings;
...
use CMU::WebInt::S_P_status::vars qw($device $chassis $cfgobjs $typemap);
# Use subclasses
...
#      warn __FILE__ . ":" . __LINE__ . ": " . Data::Dumper->Dump([$self],[qw(self)]) . "\n"; 
      ...
      foreach $chassis (@{$self->{'dev_conf'}{'children'}{'container'}}) {
...
#  This is usually 2 items at the same relpos.
  foreach (keys %$conf) {
    if (($conf->{$_}{'entPhysicalClass'} =~ /^container$/) &&
	($conf->{$_}{'entPhysicalParentRelPos'} == 1) &&
	($conf->{$_}{'entPhysicalName'} =~ /^MSFC Container/ )
       ) {
...
#    warn __FILE__ . ":" . __LINE__ . ": Parts are \n\t \$_ = $_\n\t\$conf->{$_}{'entPhysicalParentRelPos'} = $conf->{$_}{'entPhysicalParentRelPos'}\n\t\$conf->{$_}{'entPhysicalContainedIn'} = $conf->{$_}{'entPhysicalContainedIn'}\n\n";
    if (defined $conf->{$conf->{$_}{'entPhysicalContainedIn'}}{children}{$conf->{$_}{'entPhysicalClass'}}[$conf->{$_}{'entPhysicalParentRelPos'}]) {
      warn __FILE__ . ":" . __LINE__ . ": get_config: Overwriting \n" .
	Data::Dumper->Dump([$conf->{$conf->{$_}{'entPhysicalContainedIn'}}{children}{$conf->{$_}{'entPhysicalClass'}}[$conf->{$_}{'entPhysicalParentRelPos'}]],[qw(lost_child)]) .
	    "With\n" .
	      Data::Dumper->Dump([$conf->{$_}],[qw(new_child)]) .
		  "in class \n"; 
    }
    $conf->{$conf->{$_}{'entPhysicalContainedIn'}}{children}{$conf->{$_}{'entPhysicalClass'}}[$conf->{$_}{'entPhysicalParentRelPos'}] = $conf->{$_};
  }
